...
## Verify Opt-In Integration Gates With Real Environment

- **Context**: `manage-saved-symptom-check` implementation review; `npm run test:db` initially failed because `TEST_DATABASE_URL` was unset, then passed after running against the disposable local PostgreSQL database.
- **Problem**: Progress checkboxes can claim database verification while the reviewer's local environment has not actually run the opt-in integration gate.
- **Rule**: When a plan depends on opt-in integration evidence, run the command against its documented disposable environment during review or explicitly record it as unverified.
- **Applies to**: implementation reviews and release-gate verification
